...
public class VertexAO : MonoBehaviour
{
    ...
    [Tooltip("UV channel to save bent normals (rgb) and coverage (a) into.")]
    [Range(0, 7)]
    [SerializeField] private int uvChannelIndex = 3;
    ...
    private MeshFilter MyMeshFilter
    {
        get
        {
            ...
            if (_meshFilter == null)
            {
                UnityEngine.Debug.LogWarning($"{nameof(VertexAO)} requires a MeshFilter, disabling.");
                enabled = false;
            }
            return _meshFilter;
        }
    ...
    public void GatherSamples()
    {
        if (!enabled)
        {
            UnityEngine.Debug.LogWarning($"{nameof(VertexAO)} can't gather samples while component is disabled.");
            return;
        }
        ...
        for (int vi = 0; vi < mesh.vertexCount; vi++)
        {
            ...
            var coverage = 1 - ((float)_hitsInHemisphere.Count / SamplesPerVertex);
        ...
        // Save the results to the mesh spare UV channel
        mesh.SetUVs(uvChannelIndex, _bentNormalsAo);

        // convert _bentNormalsAo a list of colors
        var colors = new List<Color>();
        foreach (var bn in _bentNormalsAo)
        {
            colors.Add(new Color(bn.x, bn.y, bn.z, bn.w));
        }
